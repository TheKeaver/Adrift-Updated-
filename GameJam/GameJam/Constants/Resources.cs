namespace GameJam.Constants
{
    public class Resources
    {
        public static readonly string TEXTURE_PLAYER_SHIP = "textures/PlayerShip";
        public static readonly string TEXTURE_PLAYER_SHIELD = "textures/shield";
        public static readonly string TEXTURE_EXPLOSION = "textures/Explosion";
        public static readonly string TEXTURE_KAMIKAZE = "textures/Kamikaze";
        public static readonly string TEXTURE_SHOOTER_ENEMY = "textures/ShootingEnemy";
        public static readonly string TEXTURE_ENEMY_BULLET = "textures/EnemyBullet";

		public static readonly string TEXTURE_TITLE_WITH_INSTRUCTIONS = "textures/TitleInstructions";
		public static readonly string TEXTURE_TITLE_WITHOUT_INSTRUCTIONS = "textures/TitleNoInstructions";

		public static readonly string SOUND_EXPLOSION = "sounds/Explosion_Sound";
        public static readonly string SOUND_PROJECTILE_FIRED = "sounds/Laser_Shot";
        public static readonly string SOUND_PROJECTILE_BOUNCE = "sounds/Proj_Bounce";
    }
}
